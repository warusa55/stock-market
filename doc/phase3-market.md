...
銘柄コードからTDnet適時開示の最新候補を取得し、入力欄またはマイ棚保存へ流す
取得候補の実行結果、候補なし、取得失敗理由を入力/概要画面で確認する
```

TDnet候補取得は、開示一覧の取得元としてYanoshin TDnet WEB-APIを使う。書類本文そのものではなく、開示タイトル、会社名、開示日時、書類URLなどのインデックス情報を取り込む。
