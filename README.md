...
- 設定画面から手動見出しを追加し、localStorage の見出しを優先する
- 危険語・注目語辞書で見出しを分類する
...
公開データは `public/data/feeds.json` に置きます。個人データである登録銘柄、応援理由、怖い点、確認状態、ユーザーメモ、手動見出しはブラウザの localStorage に保存します。個人データは外部送信しません。

`feeds.json` と localStorage の手動見出しに同じID、URL、タイトル/会社名/コードの重複がある場合は、localStorage 側を優先します。
...
GitHub Actions が `npm run build:feeds` を実行し、`config/rss-feeds.json` に登録された公開RSS等から取得できる見出しを集約して `public/data/feeds.json` を生成します。RSS取得に失敗しても、動作確認用のサンプル見出しを必ず混ぜます。

GitHub Actions は個人の登録銘柄ではなく、RSSフィード一覧を入力にします。ブラウザ側は `feeds.json` を読み、localStorage の登録銘柄と突合します。ユーザーの localStorage データを GitHub Actions に渡しません。

`config/rss-feeds.json` の例:

```json
[
  {
    "id": "tdnet-yanoshin-recent",
    "name": "TDnet WEB-API recent RSS",
    "source": "tdnet",
    "kind": "rss",
...
手元で見出しを足したい場合は、設定画面の「手動見出し」から追加します。追加した見出しは localStorage に保存され、Dashboard では `feeds.json` の見出しとまとめて照合されます。
